...
import { API_ARTICLES_URL, API_CATEGORIES_URL, API_INSTANCE_URL, INSTANCE_ENTITY_NAME } from "../constants";
import { EListEntityName, IArticle, IGetArticlesReqParams, IGetEntitiesListPromiseValue, IGetInstancePromiseValue } from "./type";

const getEntitiesList = async function<T>(requestUrl: string, entityName: EListEntityName): Promise<IGetEntitiesListPromiseValue<T>> {
...
const getArticlesListService = async (params: IGetArticlesReqParams) => {
    ...
    console.log('getArticlesListService params:', params);
    const url = API_ARTICLES_URL
        + '?search=1'
...
const getCategoriesListService = async () => {
    return getEntitiesList<IArticle>(API_CATEGORIES_URL, EListEntityName.Category);
}
